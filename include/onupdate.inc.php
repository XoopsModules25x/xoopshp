...
if ($ref === '' || strpos($ref, XOOPS_URL . '/modules/system/admin.php') === 0) {
    ...
    if ($xoopsDB->query('SELECT display FROM ' . $xoopsDB->prefix($module_prefix . '_quiz')) === false) {
        ...
        // quiz database
        $result = $xoopsDB->queryF('ALTER TABLE ' . $xoopsDB->prefix($module_prefix . '_quiz') . " ADD   display TINYINT NOT NULL DEFAULT  '1'");
        if ($result) {
        ...
        $result = $xoopsDB->queryF('ALTER TABLE ' . $xoopsDB->prefix($module_prefix . '_quiz') . " ADD  expire DATETIME NOT NULL DEFAULT  '0000-00-00 00:00:00'");
        if ($result) {
        ...
        // sections database
        $result = $xoopsDB->queryF('ALTER TABLE ' . $xoopsDB->prefix($module_prefix . '_sections') . " ADD   display TINYINT NOT NULL DEFAULT  '1'");
        if ($result) {
            echo '<br>' . _AM_MSG_UPDATE_FAILED . ' (' . $xoopsDB->prefix($module_prefix . '_sections') . '.display) <br>';
            $errors++;
        } else {
            //			echo _AM_MSG_UPDATE_SUCCEEDED." (".$xoopsDB->prefix($module_prefix."_sections").".display) <br>";
        }

        $result = $xoopsDB->queryF('ALTER TABLE ' . $xoopsDB->prefix($module_prefix . '_sections') . " ADD  expire DATETIME NOT NULL DEFAULT  '0000-00-00 00:00:00'");
        if ($result) {
